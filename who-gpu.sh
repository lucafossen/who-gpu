...
# ---- --web ----------------------------------------------------------------
# The dashboard is two files: a shell page copied once per run, and a data
# file the probe loop rewrites. The page pulls new data in by injecting a
# <script src> at it -- script tags ignore the same-origin policy, which is
# what makes this work from file:// where fetch() is blocked. No server, no
# dependencies.

# The shell page lives in fleet.html next to this script, so it can be edited
# as HTML; it never changes while running, so it is copied once per run.
write_shell_html() {
  local src; src="$(resolve_self_dir)/fleet.html"
  [[ -r "$src" ]] || { echo "who-gpu: cannot find fleet.html next to who-gpu.sh ($src)" >&2; exit 1; }
  cp "$src" "$WEB_OUT/fleet.html"
}
